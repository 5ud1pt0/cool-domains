...
contract Domains is ERC721URIStorage {
  ...
  event DomainMinted(address sender, uint256 tokenId);

  error Unauthorized();
  error AlreadyRegistered();
  error InvalidName(string name);

constructor(string memory _tld) ERC721 ("Dada Name Service", "DDNS") payable {
  owner = payable(msg.sender);
  tld = _tld;
  console.log("Deploying %s name service ", _tld);
}
  ...
  function register(string calldata name) public payable {
      ...
      bytes(
        string(
          abi.encodePacked(
            '{"name": "',
            _name,
            '", "description": "A domain on the dada name service", "image": "data:image/svg+xml;base64,',
    ...
    _tokenIds.increment();
        emit DomainMinted(msg.sender, newRecordId);
  }
